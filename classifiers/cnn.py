...
class CNN:
	...
	def optimize(self, datagen, epoch_count):
		...
		for train_indexes, test_indexes in kf.split(X):
			...
			res = self.model.fit(X_train, y_train, batch_size=self.minibatch_size, shuffle=False, validation_data=(X_test, y_test), epochs=epoch_count, callbacks=[cb])
	...
	def classify(self, threads, data_gen, classification_type):
		...
		for X_train, y_train, train_info, X_test, y_test, test_info, iteration, max_iter in data_gen:
			...
			es = EarlyStopping(monitor="val_acc", verbose=1, patience=400)
